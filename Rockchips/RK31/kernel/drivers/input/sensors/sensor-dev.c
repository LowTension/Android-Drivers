 ...
 *
 */
//#define DEBUG /* Enable GMA debug data .*/
...
static int sensor_get_id(struct i2c_client *client, int *value)
{
	...
	if(sensor->ops->id_i2c == ACCEL_ID_GMA302){
		msleep(30);
		/* 1. Powerdown reset */
		buffer[0] = 0x01;
		buffer[1] = 0x02;
				      ...
				      size_t count)
{
	...
	/* save file */
	sensor->zdirection = 0; //Manual calibration will start Analyzing zdirection flag
	sensor->boot_times = 0; //Manual calibration will start Analyzing zdirection
	//GMA_WriteCalibration(sensor, GMA_Offset_TXT);
	return count;
}
	...
	size_t count)
{
	...
	sensor->zdirection = direction;
	dev_info(&sensor->client->dev, "zdirection=%ld, sensor->zdirection=%d :%d\n",direction, sensor->zdirection, __LINE__);
...
static DEVICE_ATTR(chipinfo,
		   ...
		   gma_chipinfo_show,
		   NULL);
static DEVICE_ATTR(zdirection, 
			S_IRUGO|S_IWUSR,
			gma_zdirection_show,
			gma_zdirection_store
			);
