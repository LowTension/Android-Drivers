...
//#define _GMA303_SUPPORT_AUTO_DETECT_
...
static struct i2c_board_info __initdata i2c_gma303={ I2C_BOARD_INFO("gma303", 0x18)};
...
//        .owner          = THIS_MODULE,
	...
	//.address_list= gma303_forces,
};

/*----------------------------------------------------------------------------*/
static struct i2c_client *gma30x_i2c_client = NULL;
#ifdef _GMA303_SUPPORT_AUTO_DETECT_
    static struct sensor_init_info  gma303_init_info =
    {
        .name   = GMA30x_DEV_NAME,
...
static int gma30x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = gma303_get_cust_acc_hw(); /*for auto detect feature*/
...
#ifdef _GMA303_SUPPORT_AUTO_DETECT_
    if((err = gma30x_create_attr(&gma303_init_info.platform_diver_addr->driver)))
#else
...
static int gma30x_i2c_remove(struct i2c_client *client)
{
	int err = 0;	
#ifdef _GMA303_SUPPORT_AUTO_DETECT_
    if((err = gma30x_delete_attr(&gma303_init_info.platform_diver_addr->driver)))
#else
...
{
	struct acc_hw *hw = gma303_get_cust_acc_hw();
	GSE_FUN();
...
#else    // else from _GMA303_SUPPORT_AUTO_DETECT_
/*****************************************
 *** gma30x_remove
 *****************************************/
static int gma30x_remove(void)
...
#ifdef _GMA303_SUPPORT_AUTO_DETECT_
    hwmsen_gsensor_add(&gma303_init_info);
#else
