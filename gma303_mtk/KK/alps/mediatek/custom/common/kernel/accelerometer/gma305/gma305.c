...
extern struct acc_hw* gma305_get_cust_acc_hw(void); 

/*----------------------------------------------------------------------------*/
#define I2C_DRIVERID_GMA30x 305
/*----------------------------------------------------------------------------*/

//#define _GMA305_SUPPORT_AUTO_DETECT_
...
static const struct i2c_device_id gma305_i2c_id[] = {{GMA30x_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_gma305={ I2C_BOARD_INFO("gma305", 0x18)};
...
#ifdef _GMA305_SUPPORT_AUTO_DETECT_
...
#endif
	.id_table = gma305_i2c_id,
};

/*----------------------------------------------------------------------------*/
static struct i2c_client *gma30x_i2c_client = NULL;
#ifdef _GMA305_SUPPORT_AUTO_DETECT_
    static struct sensor_init_info  gma305_init_info =
    {
        .name   = GMA30x_DEV_NAME,
...
static void GMA30x_power(struct acc_hw *hw, unsigned int on) 
{
...
/*
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "GMA305"))
			{
		...
		{
			if (!hwPowerDown(hw->power_id, "GMA305"))
			{
...
static int gma30x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = gma305_get_cust_acc_hw(); /*for auto detect feature*/
...
#ifdef _GMA305_SUPPORT_AUTO_DETECT_
    if((err = gma30x_create_attr(&gma305_init_info.platform_diver_addr->driver)))
#else
...
static int gma30x_i2c_remove(struct i2c_client *client)
{
	int err = 0;	
#ifdef _GMA305_SUPPORT_AUTO_DETECT_
    if((err = gma30x_delete_attr(&gma305_init_info.platform_diver_addr->driver)))
#else
...
#ifndef _GMA305_SUPPORT_AUTO_DETECT_
...
{
	struct acc_hw *hw = gma305_get_cust_acc_hw();
	GSE_FUN();

	GMA30x_power(hw, 1);
	//gma303_force[0] = hw->i2c_num;//modified
...
#else    // else from _GMA305_SUPPORT_AUTO_DETECT_
/*****************************************
 *** gma30x_remove
 *****************************************/
static int gma30x_remove(void)
...
/*----------------------------------------------------------------------------*/
static int __init gma305_init(void)
{
	//GSE_FUN();
	struct acc_hw *hw = gma305_get_cust_acc_hw();
	GSE_LOG("%s: i2c_number=%d\n", __func__,hw->i2c_num); 
	i2c_register_board_info(hw->i2c_num, &i2c_gma305, 1);
#ifdef _GMA305_SUPPORT_AUTO_DETECT_
    hwmsen_gsensor_add(&gma305_init_info);
#else
...
/*----------------------------------------------------------------------------*/
static void __exit gma305_exit(void)
{
	GSE_FUN();
#ifndef _GMA305_SUPPORT_AUTO_DETECT_
...
/*----------------------------------------------------------------------------*/
module_init(gma305_init);
module_exit(gma305_exit);
/*----------------------------------------------------------------------------*/
MODULE_LICENSE("GPL");
