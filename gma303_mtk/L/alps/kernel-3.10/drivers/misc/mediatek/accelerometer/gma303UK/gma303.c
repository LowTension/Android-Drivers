...
static int GMA30x_CheckDeviceID(struct i2c_client *client)
{
	...
	if(databuf[0] == GMA30x_VAL_WMI)
		GSE_LOG("%s: PID = 0x%x, Globalmems accelerometer\n", __func__, databuf[0]);
...
static int GMA30x_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		if( ABS(acc[GMA30x_AXIS_X]) < LevelValueRange_0_39 && ABS(acc[GMA30x_AXIS_Y]) < LevelValueRange_0_39)
		{
			acc[GMA30x_AXIS_X] = 0;
			acc[GMA30x_AXIS_Y] = 0;
		}
		...
		acc[GMA30x_AXIS_Z] = acc[GMA30x_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;		
				...
				//Out put the mg
		acc[GMA30x_AXIS_Z]<<=1;//GMA303KU
#ifdef USE_VirtualZ
		acc[GMA30x_AXIS_Z]= VirtualZ((GRAVITY_EARTH_1000*GRAVITY_EARTH_1000) - (acc[GMA30x_AXIS_X] * acc[GMA30x_AXIS_X]) - (acc[GMA30x_AXIS_Y] * acc[GMA30x_AXIS_Y]), acc[GMA30x_AXIS_Z] );
#endif
