...
static struct acc_init_info gma305_init_info = {
	.name   = GMA30x_DEV_NAME,
...
static int gma30x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = gma30x_create_attr(&gma305_init_info.platform_diver_addr->driver);
    if(err != 0) {
...
static int gma30x_i2c_remove(struct i2c_client *client)
{
	...
	err = gma30x_delete_attr(&gma305_init_info.platform_diver_addr->driver);
    if (err != 0) {
...
static int __init gma305_init(void)
{
	...
	acc_driver_add(&gma305_init_info);
